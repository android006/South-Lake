...
import Util from '../../components/Request/Util';
...
class Filtrate extends Component {
  ...
  render () {
    ...
    return (
      <div className={styles.wpComponentsBgc}>
        <Form className={styles.tML10} layout="inline" key="myForm">
          {
            items.map((item, index) => {
              if (item.type === 'input') {
                return (<FormItem {...formItemLayout} label={item.label} key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                      ...
                      <Input onChange={t.getChangeValue.bind(t, item.paramName)} size={config.size} placeholder={item.placeholder}/>
                    )
                  }
                </FormItem>)
              } else if (item.type === 'select') {
                return (<FormItem {...formItemLayout} label={item.label} className={styles.selectWidth} key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                ...
                return (<FormItem {...formItemLayout} className={styles.rangePicker} label={item.label}
                                  key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                ...
                return (<FormItem {...formItemLayout} className={styles.monthPicker} label={item.label}
                                  key={index}>
                  {
                    getFieldDecorator(item.paramName, {
                      initialValue: item.initialValue ? item.initialValue : moment()
                ...
                return (
                  <FormItem {...formItemLayout}
                            className={styles.rangePicker} label={item.label}
                            key={index}>
                    {
                      getFieldDecorator(item.paramName, {
          ...
          <FormItem className={styles.wpBtnRow}>
          ...
          </FormItem>
          {
            extraBtn.map((btn, btnIndex) => {
              return (
                <FormItem key={btnIndex} className={styles.wpBtnRow}>
                  <Button size={config.size}
                          className={styles.wpBtn}
                          type="primary"
...
export default  Form.create({withRef: true})(Filtrate);
